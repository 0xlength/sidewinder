...
sidewinder depends on many external dependencies including optional C libraries.
...
sidewinder requires further requires Go 1.20. Other Go versions are not supported.
...
Once your Go toolchain and build dependencies are installed, you can build sidewinder as usual:

    go mod download
    go run ./cmd/sidewinder

## Building with deps.sh

`deps.sh` fetches deps using Git, compiles them, and installs them into the `opt` dir of your sidewinder checkout.
...
sidewinder provides a [Nix](https://nixos.org/) package.

    nix-build
    ./result/bin/sidewinder --help
...
To create a pure-Go build use the `lite` tag.
    ...
    go build -tags=lite ./cmd/sidewinder
