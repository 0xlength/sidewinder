...
# Change into sidewinder root directory
...
cat <<EOF
  ...
  deps.sh is a lightweight dependency manager for sidewinder.
...
if [[ $# -eq 0 ]]; then
  echo "[~] This will fetch, build, and install sidewinder dependencies into $(pwd)/opt"
